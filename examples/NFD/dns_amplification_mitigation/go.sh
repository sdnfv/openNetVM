...
then
  config=$2
  shift 2
  exec sudo $SCRIPTPATH/build/DNSAmplificationMitigation -F $config "$@"
elif [ $1 = "-F" ]
then
  usage
fi
...
exec sudo $SCRIPTPATH/build/app/DNSAmplificationMitigation -l $cpu -n 3 --proc-type=secondary -- -r $service $instance -- -d $dst $print
