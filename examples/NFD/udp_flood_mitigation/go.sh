...
then
  config=$2
  shift 2
  exec sudo $SCRIPTPATH/build/UDPFloodMitagation -F $config "$@"
elif [ $1 = "-F" ]
then
  usage
fi
...
exec sudo $SCRIPTPATH/build/app/UDPFloodMitagation -l $cpu -n 3 --proc-type=secondary -- -r $service $instance -- -d $dst $print
